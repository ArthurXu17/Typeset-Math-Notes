...
\bhard {
    Let $f, g : \bN \to \bN$ be functions. Define the set $B(f, g) = \{ k \in \bN : f(k) = g(k) \}$. That is, $B(f, g)$ is the set of numbers on which $f$ and $g$ agree. Construct a family $\{f_{n}\}_{n\in\bN}$ of functions $f_{n}:\bN \to \bN$ with the property that for any $g : \bN \to \bN$ and any $N \in \bN$, there is an $f_{n}$ in the family such that $\lvert B(f_{n}, g)\rvert > N$.
} {
...
\bhard {
    We have already proved that $(0, 1)$ is uncountable and that $\cp(\bN)$ is uncountable. Prove that these two sets are of the same cardinality. That is, construct a bijection $f : \cp(\bN) \to(0, 1)$ as explicitly as possible.
} {
...
\bhard {
    The following is a beautifully slick proof that the real numbers are uncountable. 

    \vspace{1em}
    We define a game for two players. Ahead of time, fix a subset $A \subseteq [0, 1]$. 
    
    \vspace{1em}
    Player 1 starts by choosing a number $a_{1} \in (0, 1)$. Player 2 responds by choosing a number $b_{1} \in (a_{1}, 1)$. The process then repeats inside the interval $[a_{1}, b_{1}]$: Player 1 chooses a number $a_{2} \in (a_{1}, b_{1})$, and Player 2 chooses a number $b_{2} \in (a_{2}, b_{1})$. The game continues in this way for all $n\in\bN$: at stage $n + 1$, Player 1 chooses a number $a_{n+1} \in (a_{n}, b_{n})$, and Player 2 responds by choosing a number $b_{n+1} \in (a_{n+1}, b_{n})$.
    
    \vspace{1em}
    At the end of the game, the two players have created a sequence of nested, closed intervals $C_{n} = [a_{n}, b_{n}]$. The sequence $\{a_{n}\}$ is increasing by construction and bounded above (by 1, for example), and so it converges by the Monotone Sequence Theorem. Let $a$ be its limit. We say that Player 1 wins the game if $a \in A$, and Player 2 wins the game if $a \not\in A$. Prove that if $A$ is countable, then Player 2 has a winning strategy. Then prove as an immediate corollary that $[0, 1]$ is uncountable.
} {
    First we prove a quick claim that $a < b_{n}$ for all $n\in \bN$. First we show that $a\leq b_{n}$. Suppose for contradiction that there exists an $n\in\bN$ such that $a > b_{n}$. Consider $\epsilon = \frac{a-b_{n}}{2} > 0$. By the definition of convergence, there exists an $m\in \bN$ such that $\lvert a_{m} - a\rvert < \frac{a-b_{n}}{2} \implies a_{m} > a - \frac{a-b_{n}}{2} = \frac{a}{2} + \frac{b_{n}}{2} > \frac{b_{n}}{2} + \frac{b_{n}}{2} = b_{n}$. Thus we have reached that $a_{m} > b_{n}$ which contradicts the fact that the intervals $[a_{i}, b_{i}]$ are nested (i.e $a_{i} < b_{j}$ for all $i,j\in\bN$). Thus we have that $a\leq b_{n}$ for all $n\in \bN$. We then quickly note that we can't have $a = b_{n}$ either because $b_{n} > b_{n+1}$ and so we get that $a > b_{n+1}$ a contradiction. Thus $a < b_{n}$ for all $n\in \bN$.
...
\beasy [t2t1t0]{
    Prove that every Hausdorff space is $T_{1}$, and that every $T_{1}$ space is $T_{0}$.
} {
...
\beasy [T1uniqueconstantlimit]{
    Let $(X, \ct)$ be a $T_{1}$ topological space, and let $x \in X$. Show the constant sequence $x, x, x, x, \dots$ converges to $x$ and to no other point.
} {
...
\beasy [t1equiv]{
    Let $(X, \ct )$ be a topological space. Prove that the following are equivalent.
    \renewcommand{\labelenumi}{(\arabic{enumi})}
    \begin{enumerate}
        \item $X$ is $T_{1}$
        \item Every constant sequence in X converges only to its constant value.
        \item For all $x\in X$, $\{x\}$ is closed
        \item Every finite subset of X is closed.
        \item For every subset $A \subseteq X$, $A = \cap \{U \subseteq X : U \text{ is open and } A \subseteq U \}$.
    \end{enumerate}
    \renewcommand{\labelenumi}{\alph{enumi})}
} {
    $(1)\implies (2)$
...
\beasy [cocountableNotFirstCountable] {
    Let $X$ be uncountable. Show that $(X, \ctc)$ is not first countable.
} {
...
\beasy {
    Let $X$ be a finite set. Show that the only $T_{1}$ topology on $X$ is the discrete topology.
} {
...
\beasy [HausdorffRefine]{
    Let $X$ be a set and let $\ct_{1}$ and $\ct_{2}$ be two distinct topologies on $X$ such that $\ct_{1}\subseteq \ct_{2}$. If $(X, \ct_{1})$ is Hausdorff, does that imply that $(X, \ct_{2})$ is Hausdorff? What about the other direction? In both cases, prove it or give a counterexample.
} {
...
\bmed {
    A subset $A$ of a topological space $(X, \ct )$ is called a $G_{\delta}$ set if it equals a countable intersection of open subsets of $X$. In other words, $A \subseteq X$ is a $G_{\delta}$ set if there exist open sets $U_{n}$, $n \in \bN$, such that $A = \bigcap_{n\in\bN} U_{n}$. Show that in a first countable $T_{1}$ space, $\{x\}$ is a $G_{\delta}$ set for every $x \in X$.
} {
...
\bmed [nestedLocalBasis] {
    Let $(X, \ct )$ be a first countable topological space. Show that every $x \in X$ has a countable nested local basis. That is, for every $x \in X$, show that there is a local basis $B_{x} = \{ B_{n} : n \in \bN \}$ such that $B_{1} \supseteq B_{2} \supseteq B_{3} \supseteq \cdots$
} {
...
\bmed {
    Let $(X, \ct )$ be a first countable topological space, and let $A \subseteq X$. Prove that $x \in \overline{A}$ if and only if there is a sequence of elements of $A$ converging to $x$.
} {
...
\bmed [CountableSpaceSepCCC] {
    Is every countable topological space $X$ (ie. one in which the underlying set is countable) separable? What about ccc? Can a countable space be first countable but not second countable?
} {
...
\bhard {
    Construct a topological space $(X, \ct )$ which is countable (ie. $X$ is countable) but not first countable.
} {
